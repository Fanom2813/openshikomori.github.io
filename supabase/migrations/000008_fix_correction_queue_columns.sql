-- Migration: Fix column count mismatch in get_clips_for_correction_v2
-- This adds the transcription_history column to the return type to match the clips table structure

-- PostgreSQL does not allow changing the return type with CREATE OR REPLACE, 
-- so we must drop it first.
DROP FUNCTION IF EXISTS get_clips_for_correction_v2(UUID, TEXT, INTEGER);
